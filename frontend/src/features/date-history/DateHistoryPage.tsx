  ...
  return (
    <QueryBoundary query={query}>
      {(data) => {
        ...
        const openResult = (dateId: string) =>
          navigate(`/matches/${matchId}/result`, {
            state: {
              dateId,
              partnerAgentId:
                locState.partnerAgentId || data.match.partnerAgent.agentId || undefined,
              partnerName: locState.partnerName || data.match.partnerAgent.displayName,
              partnerAvatarUrl: locState.partnerAvatarUrl || data.match.partnerAgent.avatarUrl,
            },
          });
        const title = locState.partnerName || data.match.partnerAgent.displayName;
        return (
          <>
            <MobileHeader showBack title={title} />
            <div className="flex-1 min-h-0 overflow-y-auto px-4 pt-2 pb-6 space-y-3">
              {data.dates.map((d) => (
                <DateHistoryCard key={d.dateId} item={d} onRate={() => openResult(d.dateId)} />
              ))}
            </div>
          </>
        );
...
function DateHistoryCard({ item, onRate }: { item: DateHistoryItem; onRate: () => void }) {
  ...
  return (
    <article className="rounded-2xl bg-surface shadow-card p-4 flex flex-col gap-2">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-h3 font-bold tracking-tight">{typeLabel[item.type] ?? item.type}</h3>
